...
server <- function(input, output, session) {
  ...
  output$dag_plot <- renderPlot({
    ...
    text((nx+yx)/2, ny+0.12,
         sprintf("r = %+.2f", r_xy), cex=0.74, col="#2c2c2a", font=2)
    ...
    draw_node(nx, ny, x_lab(), fill="#eeedfe", border="#534ab7", txt_col="#2c2c2a")
    draw_node(yx, yy, y_lab(), fill="#eeedfe", border="#534ab7", txt_col="#2c2c2a")
